...
def old_clean(ts, dtrange=[0, 250], t=5):
    ...
    for i in range(1, s.size-1):
        vb = (ts.iloc[i-1]-minVal)/(maxVal-minVal)*100
        va = (ts.iloc[i+1]-minVal)/(maxVal-minVal)*100
        out = (ts.iloc[i]-minVal)/(maxVal-minVal)*100
